...
from py_dbmigration.data_file_mgnt.data_files import DataFile,ChildFOI 
...
# def extract_file(self, db, abs_file_path, abs_writable_path, skip_ifexists=False):
    ...
    # We walk the tmp dir and add those data files to list of to do
        ...
        file_table_map = [ChildFOI(file_type='DATA', file_regex='.*', file_path=modified_write_path, file_name_data_regex=None,
                                                                    parent_file_id=file_id, project_name=foi.project_name)]

        # instantiate a new Datafile object that crawl this new directory of extracted files
        DataFile(
