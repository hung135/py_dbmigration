...
def load_csv(args):
    ...
    df = pd.read_excel(file,  sheet_name=int(args.s),index_col=None, usecols=None, header=int(args.h),engine='openpyxl')
...
def main():
    ...
    parser.add_argument('--s' , help='sheet number',default='1')
    parser.add_argument('--h' , help='header row location',default='0')
    parser.add_argument('--v' , help='print version info',action="store_true")

    parser.add_argument('--tag' , help='add a TAG column name to dataframe')
    parser.add_argument('--cols' , help='column names to use')
